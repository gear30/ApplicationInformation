...
namespace Application.DTO.Repositories
{
    public class MockApplicationRepository : IApplicationRepository
    {
        ...
        public int AddInformation(Information information)
        {
            ...
            return _informations.Select(x => x.ID).Max();
        }

        public bool CheckCountryPermitted(string CountryName)
        {
            return _countries.Where(x => x.CountryName == CountryName && x.Permitted).Any();
        }

        public Information GetInforation(int informationId)
        {
            return _informations.Where(x => x.ID == informationId).FirstOrDefault();
        }

        public int Updatenformation(Information information)
        {
            var old = GetInforation(information.ID);
            old.InformationAddress = information.InformationAddress;
            old.CountryOfResidence = information.CountryOfResidence;
            old.DateOfBirth = information.DateOfBirth;
            old.FirstName = information.FirstName;
            old.InformationOccupation = information.InformationOccupation;
            old.LastName = information.LastName;
            old.Title = information.Title;
